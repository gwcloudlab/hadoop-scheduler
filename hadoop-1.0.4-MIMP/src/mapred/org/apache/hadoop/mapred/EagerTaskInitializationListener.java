...
class EagerTaskInitializationListener extends JobInProgressListener {
  ...
  private synchronized void resortInitQueue() {
	...
	System.out.printf("###current time: %d, job name: %s, jobID: %s, job startTime: %d, job launchTime: %d, job relativeDeadline: %d, job deadline: %d%n", 
                         System.currentTimeMillis(), job.getProfile().getJobName(), job.getJobID().toString(), job.getStartTime(), job.getLaunchTime(), job.getJobRelativeDeadline(), job.getJobDeadline());
      }
    }
  }
