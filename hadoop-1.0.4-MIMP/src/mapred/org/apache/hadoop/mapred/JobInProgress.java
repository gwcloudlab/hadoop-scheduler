...
public class JobInProgress {
 ...
 public void putTaskInfo(Task task, double predictTime, String hostname) {
   ...
   d.IO[3] = JobQueueTaskScheduler.resources.get("slave03").getDisk();
   d.IO[4] = JobQueueTaskScheduler.resources.get("slave04").getDisk();
   d.IO[5] = JobQueueTaskScheduler.resources.get("slave05").getDisk();
        ...
        if(state == TaskStatus.State.SUCCEEDED || state == TaskStatus.State.FAILED || state == TaskStatus.State.KILLED){
          if(status.getIsMap()){
            if(!tip.isJobCleanupTask() && !tip.isJobSetupTask()){
              ...
              System.out.printf("JobName=%s, ID=%s, Type=MAP, Status=%s, Machine=%s, StartTime=%s, FinishTime=%s, ExecTime=%d, PredictTime=%f, sharedCPU=%f, IO1=%f, IO2=%f, IO3=%f, IO4=%f, IO5=%f, IO6=%f %n ",
                                tip.getJob().getProfile().getJobName(), taskid, status.getRunState(), machine,
                                status.getStartTime(), status.getFinishTime(), status.getFinishTime()-status.getStartTime(), d.predictTime, d.sharedCPU, d.IO[0], d.IO[1], d.IO[2], d.IO[3], d.IO[4], d.IO[5]);
